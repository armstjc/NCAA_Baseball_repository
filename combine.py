...
def load_ncaa_roster():
    print('Loading in NCAA Rosters')

    main_df = pd.DataFrame()
    season_df = pd.DataFrame()
    current_year = datetime.now().year
    for i in range(2013,current_year+1):
        print(f'Loading in the {i} roster.')
        try:
            season_df = pd.read_csv(f'TeamRosters/{i}_roster.csv')
            main_df = pd.concat([main_df,season_df])
        except:
            print(f'Could not load in the roster file for the {i} season. The file may not exist, or it may be in use.')

    main_df = main_df.dropna(subset=['player_id'])
    print('Finished loading and combining rosters.')
    return main_df
...
def add_divisions():
    for i in tqdm(range(2013,2024)):
        ...
        try:
            df = df.filter(items=['R', 'AB', 'H', '2B', '3B', 'TB', 'HR', 'RBI', 'BB', 'HBP', 'SF', 'SH', 'K', 'DP', 'CS', 'Picked', 'SB', 'IBB', 'date', 'field', 'season_id', 'opponent_id', 'opponent_name', 'innings_played', 'extras', 'runs_scored', 'runs_allowed', 'run_difference', 'result', 'game_id', 'school_id', 'stats_player_seq', 'season', 'OPP DP', 'GDP','score'])
        except:
    ...
    for i in tqdm(range(2013,2024)):
        ...
        try:
            df = df.filter(items=['App','GS','IP','CG','H','R','ER','BB','SO','SHO','BF','P-OAB','2B-A','3B-A','Bk','HR-A','WP','HB','IBB','Inh Run','Inh Run Score','SHA','SFA','pitches','GO','FO','W','L','SV','OrdAppeared','KL','date','field','season_id','opponent_id','opponent_name','innings_played','extras','runs_scored','runs_allowed','run_difference','result','game_id','school_id','stats_player_seq','season','pickoffs','score','Pitches','player_id'])
        except:
            print('Dataframe was ready for divisions to be added.')
        ...
        df.to_parquet(f'game_stats/player/fielding_game_stats/parquet/{i}_pitching.parquet')
