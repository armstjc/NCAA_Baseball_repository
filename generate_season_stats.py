...
def generate_season_team_batting_stats(season:int):
    ...
    for i in range(1,5):
        ...
        del part_df
    main_df['G'] = 1
    finished_df = pd.DataFrame(main_df.groupby(['season','season_id','school_id'],as_index=False)['G','AB','H','TB','R','2B','3B','HR','RBI','BB','HBP','SF','SH','K','DP','SB','CS','Picked','IBB','OPP DP','GDP'].sum())
    ## Plate Appearances
    finished_df['PA'] = finished_df['AB'] + finished_df['BB'] + finished_df['SF'] + finished_df['SH']
    ## Batting Average
    finished_df['BA'] = finished_df['H'] / finished_df['AB']
...
def generate_season_team_pitching_stats(season:int):
    ...
    main_df = main_df.astype({'whole_innings':'int','part_innings':'int'})
    main_df['IP'] = main_df['whole_innings'] + (main_df['part_innings']/3)
    finished_df = pd.DataFrame(main_df.groupby(['season','season_id','school_id'],as_index=False)\
        ['App','IP','H','R','ER','BB','SO','SHO','BF','P-OAB','2B-A','3B-A','Bk','HR-A','WP','HB','IBB','Inh Run','Inh Run Score','SHA','SFA','Pitches','pitches','GO','FO','W','L','SV','KL','CG','pickoffs'].sum())
