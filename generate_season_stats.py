...
def generate_league_batting_stats():
    ...
    finished_df = pd.DataFrame(
        main_df.groupby(["season", "season_id", "division"], as_index=False)[
            [
                "G",
                "AB",
                "R",
    ...
    finished_df["RC"] = (
        ...
        * (
            finished_df["TB"]
            + (0.26 * (finished_df["BB"] - finished_df["IBB"] + finished_df["HBP"]))
        )
        + (0.52 * (finished_df["SH"] + finished_df["SF"] + finished_df["SB"]))
    ) / (
        finished_df["AB"]
        + finished_df["BB"]
        + finished_df["HBP"]
    ...
    ## Plate Appearances
    finished_df["PA"] = (
        finished_df["AB"] + finished_df["BB"] + finished_df["SF"] + finished_df["SH"]
    )
    ...
    finished_df["SecA"] = (
        finished_df["BB"]
        + (finished_df["TB"] - finished_df["H"])
        + (finished_df["SB"] - finished_df["CS"])
    ) / finished_df["AB"]
    ...
    finished_df["XR"] = (
        ...
        + (finished_df["3B"] * 1.04)
        + (finished_df["HR"] * 1.44)
        + (0.34 * (finished_df["HBP"] + finished_df["TB"] + finished_df["IBB"]))
        + (0.25 * finished_df["IBB"])
        + (0.18 * finished_df["SB"])
        + (-0.32 * finished_df["CS"])
        + (-0.09 * (finished_df["AB"] - finished_df["H"] - finished_df["K"]))
        + (-0.098 * finished_df["K"])
        + (-0.37 * finished_df["GDP"])
        + (0.37 * finished_df["SF"])
        + (0.04 * finished_df["SH"])
    )
    ...
    finished_df["XRR"] = (
        ...
        + (1.04 * finished_df["3B"])
        + (1.44 * finished_df["HR"])
        + (0.33 * (finished_df["HBP"] + finished_df["TB"]))
        + (0.18 * finished_df["SB"])
        + (-0.32 * finished_df["CS"])
        + (-0.098 * (finished_df["AB"] - finished_df["H"]))
    )
    ...
    finished_df["XRB"] = (
        ...
        + (1.04 * finished_df["3B"])
        + (1.44 * finished_df["HR"])
        + (0.34 * (finished_df["HBP"] + finished_df["TB"]))
        + (0.18 * finished_df["SB"])
        - (-0.32 * finished_df["CS"])
        + (-0.096 * (finished_df["AB"] - finished_df["H"]))
    )
    finished_df["XRB"] = finished_df["XRB"].round(3)

    ## Power-Speed Number
    finished_df["PSN"] = (2 * finished_df["HR"] * finished_df["SB"]) / (
        finished_df["HR"] + finished_df["SB"]
    )
    ...
    # print(finished_df)
    finished_df.to_csv(
        f"season_stats/league/batting_season_stats/csv/league_batting.csv", index=False
    )
    finished_df.to_parquet(
        f"season_stats/league/batting_season_stats/parquet/league_batting.parquet",
        index=False,
    )
...
def generate_league_pitching_stats():
    ...
    finished_df = finished_df.rename(
        columns={
            "2B-A": "2B",
            "3B-A": "3B",
            "HR-A": "HR",
    ...
    finished_df["FIP_const"] = finished_df["ERA"] - (
        (
            (13 * finished_df["HR"])
            + (3 * (finished_df["BB"] + finished_df["HBP"]))
            - (finished_df["SO"])
        )
        / finished_df["IP"]
    )
    ...
    # print(finished_df)
    finished_df.to_csv(
        f"season_stats/league/pitching_season_stats/csv/league_pitching.csv",
        index=False,
    )
    finished_df.to_parquet(
        f"season_stats/league/pitching_season_stats/parquet/league_pitching.parquet",
        index=False,
    )
...
def generate_park_factors(season: int):
    ...
    home_gm_df = pd.DataFrame(
        home_gm_df.groupby(
            ["season", "season_id", "school_id", "division"], as_index=False
        )[
            ["runs_scored", "runs_allowed", "G", "AB", "H", "2B", "3B", "HR", "BB", "K"]
        ].sum()
    )
    home_gm_df = home_gm_df.rename(
        {
    ...
    finished_df = pd.merge(
        home_gm_df,
        away_gm_df,
        left_on=["season", "season_id", "school_id", "division"],
        right_on=["season", "season_id", "school_id", "division"],
        how="left",
    )

    finished_df["PF"] = 100 * (
        (
            (finished_df["home_runs_scored"] + finished_df["home_runs_allowed"])
            / finished_df["home_G"]
        )
        / (
            (finished_df["away_runs_scored"] + finished_df["away_runs_allowed"])
            / finished_df["away_G"]
        )
    )
    ...
    print(finished_df)
    finished_df.to_csv(
        f"season_stats/league/park_factors/csv/{season}_park_factors.csv", index=False
    )
    finished_df.to_parquet(
        f"season_stats/league/park_factors/parquet/{season}_park_factors.parquet",
        index=False,
    )
    # return finished_df
...
def generate_season_player_batting_stats(season: int):
    ...
    def parser(division: int, main_df: pd.DataFrame()):
        # main_df = pd.concat([main_df,part_df],ignore_index=True)
        league_df = pd.read_parquet(
            "season_stats/league/batting_season_stats/parquet/league_batting.parquet"
        )
        ...
        main_df = pd.DataFrame(
            main_df.groupby(
                ["season", "season_id", "school_id", "stats_player_seq", "division"],
                as_index=False,
            )[
                [
                    "G",
        ...
        main_df["RC"] = (
            ...
            * (
                main_df["TB"]
                + (0.26 * (main_df["BB"] - main_df["IBB"] + main_df["HBP"]))
            )
            + (0.52 * (main_df["SH"] + main_df["SF"] + main_df["SB"]))
        ) / (
            main_df["AB"]
            + main_df["BB"]
            + main_df["HBP"]
        ...
        main_df["SecA"] = (
            main_df["BB"]
            + (main_df["TB"] - main_df["H"])
            + (main_df["SB"] - main_df["CS"])
        ) / main_df["AB"]
        ...
        ## OPS+
        main_df["OPS+"] = 100 * (
            (main_df["OPS"] / lg_ops) + (main_df["SLG"] / lg_slg) - 1
        )
        ...
        main_df["XR"] = (
            ...
            + (main_df["3B"] * 1.04)
            + (main_df["HR"] * 1.44)
            + (0.34 * (main_df["HBP"] + main_df["TB"] + main_df["IBB"]))
            + (0.25 * main_df["IBB"])
            + (0.18 * main_df["SB"])
            + (-0.32 * main_df["CS"])
            + (-0.09 * (main_df["AB"] - main_df["H"] - main_df["K"]))
            + (-0.098 * main_df["K"])
            + (-0.37 * main_df["GDP"])
            + (0.37 * main_df["SF"])
            + (0.04 * main_df["SH"])
        ...
        main_df["XRR"] = (
            ...
            + (1.04 * main_df["3B"])
            + (1.44 * main_df["HR"])
            + (0.33 * (main_df["HBP"] + main_df["TB"]))
            + (0.18 * main_df["SB"])
            + (-0.32 * main_df["CS"])
            + (-0.098 * (main_df["AB"] - main_df["H"]))
        )
        ...
        main_df["XRB"] = (
            ...
            + (1.04 * main_df["3B"])
            + (1.44 * main_df["HR"])
            + (0.34 * (main_df["HBP"] + main_df["TB"]))
            + (0.18 * main_df["SB"])
            - (-0.32 * main_df["CS"])
            + (-0.096 * (main_df["AB"] - main_df["H"]))
        )
        main_df["XRB"] = main_df["XRB"].round(3)

        ## Power-Speed Number
        main_df["PSN"] = (2 * main_df["HR"] * main_df["SB"]) / (
            main_df["HR"] + main_df["SB"]
        )
...
def generate_season_player_pitching_stats(season: int):
    ...
    def parser(division: int, main_df: pd.DataFrame()):
        league_df = pd.read_parquet(
            "season_stats/league/pitching_season_stats/parquet/league_pitching.parquet"
        )
        league_df = league_df[league_df["season"] == season]
        league_df = league_df[league_df["division"] == division]
        ...
        park_factors_df = pd.read_parquet(
            f"season_stats/league/park_factors/parquet/{season}_park_factors.parquet"
        )
        ...
        main_df["ERA-"] = 100 * (
            (
                main_df["ERA"]
                + (main_df["ERA"] - (main_df["ERA"] * (main_df["PF"] / 100)))
            )
            / lg_era
        )
...
def generate_season_player_fielding_stats(season: int):
    ...
    ## Fielding Percentage
    finished_df["FLD%"] = (finished_df["PO"] + finished_df["A"]) / (
...
def generate_season_team_batting_stats(season: int):
    ...
    finished_df.to_csv(
        f"season_stats/team/batting_season_stats/csv/{season}_batting.csv", index=False
    )
    finished_df.to_parquet(
...
def generate_team_game_pitching_stats(season: int):
    ...
    # print(finished_df)
    finished_df.to_csv(
        f"game_stats/team/pitching_game_stats/csv/{season}_pitching.csv", index=False
    )
    finished_df.to_parquet(
...
def generate_team_game_fielding_stats(season: int):
    ...
    # finished_df = finished_df.mask(np.isinf(finished_df))
    finished_df.to_csv(
        f"game_stats/team/fielding_game_stats/csv/{season}_fielding.csv", index=False
    )
    finished_df.to_parquet(
