...
def get_ncaa_baseball_roster(season: int, division: int):
    """ """
    if season < 2013:
        raise SyntaxError("'season' cannot be lower than 2013.")
    elif season > date.today().year:
        raise SyntaxError(
            f"'season' cannot be greater than {date.today().year}."
        )
    ...
    if division == 0:
        ...
        raise SyntaxError(
            "The inputted value for 'division' must be 0, 1, 2, or 3."
        )
...
def get_season_ncaa_baseball_stats(season: int, division: int):
    ...
    for i in tqdm(range(count, len_player_id_arr)):
        ...
        if len(batting_df) > 0:
            batting_df.to_csv(
                "PlayerStats/Batting/" +
                f"{int(season_id_arr[i])}_{int(player_id_arr[i])}.csv",
                index=False,
            )
